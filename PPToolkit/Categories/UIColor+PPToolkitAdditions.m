...
#import "UIColor+PPToolkitAdditions.h"

const UInt32 PPColorTurquise            = 0x1abc9c;
const UInt32 PPColorGreenSea            = 0x16a085;
const UInt32 PPColorEmerland            = 0x2ecc71;
const UInt32 PPColorNephritis           = 0x27ae60;
const UInt32 PPColorPeterRiver          = 0x3498db;
const UInt32 PPColorBelizeHole          = 0x2980b9;
const UInt32 PPColorAmethyst            = 0x9b59b6;
const UInt32 PPColorWisteria            = 0x8e44ad;
const UInt32 PPColorWetAsphalt          = 0x34495e;
const UInt32 PPColorMidnightBlue        = 0x2c3e50;
const UInt32 PPColorSunflower           = 0xf1c40f;
const UInt32 PPColorTangerine           = 0xf39c12;
const UInt32 PPColorCarrot              = 0xe67e22;
const UInt32 PPColorPumpkin             = 0xd35400;
const UInt32 PPColorAlizarin            = 0xe74c3c;
const UInt32 PPColorPomegranate         = 0xc0392b;
const UInt32 PPColorClouds              = 0xecf0f1;
const UInt32 PPColorSilver              = 0xbdc3c7;
const UInt32 PPColorConcrete            = 0x95a5a7;
const UInt32 PPColorAsbestos            = 0x7f8c8d;

static NSMutableDictionary * __colorNameCache = nil;

@interface UIColor (PPToolkitAdditionsInternal)
+ (UIColor *)searchForColorByName:(NSString *)cssColorName;
@end

#pragma mark -

@implementation UIColor (PPToolkitAdditions)

+ (NSMutableDictionary *)colorNameCache {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        __colorNameCache = [[NSMutableDictionary alloc] init];
    });
    return __colorNameCache;
}
...
- (NSArray *)pp_arrayFromRGBAComponents {
	...
	CGFloat r,g,b,a;
	if (![self pp_red:&r green:&g blue:&b alpha:&a]) return nil;
...
- (BOOL)pp_red:(CGFloat *)red green:(CGFloat *)green blue:(CGFloat *)blue alpha:(CGFloat *)alpha {
...
- (UInt32)rgbHex {
	...
	CGFloat r,g,b,a;
	if (![self pp_red:&r green:&g blue:&b alpha:&a]) return 0;
...
- (UIColor *)pp_colorByMultiplyingBy:(CGFloat)f {
	return [self pp_colorByMultiplyingByRed:f green:f blue:f alpha:1.0f];
}

- (UIColor *)pp_colorByAdding:(CGFloat)f {
	return [self pp_colorByMultiplyingByRed:f green:f blue:f alpha:0.0f];
}

- (UIColor *)pp_colorByLighteningTo:(CGFloat)f {
	return [self pp_colorByLighteningToRed:f green:f blue:f alpha:0.0f];
}

- (UIColor *)pp_colorByDarkeningTo:(CGFloat)f {
	return [self pp_colorByDarkeningToRed:f green:f blue:f alpha:1.0f];
}
...
- (UIColor *)pp_colorByMultiplyingByColor:(UIColor *)color {
	...
	return [self pp_colorByMultiplyingByRed:r green:g blue:b alpha:1.0f];
}
...
- (UIColor *)pp_colorByAddingColor:(UIColor *)color {
	...
	return [self pp_colorByAddingRed:r green:g blue:b alpha:0.0f];
}
...
- (UIColor *)pp_colorByLighteningToColor:(UIColor *)color {
	...
	return [self pp_colorByLighteningToRed:r green:g blue:b alpha:0.0f];
}
...
- (UIColor *)pp_colorByDarkeningToColor:(UIColor *)color {
	...
	return [self pp_colorByDarkeningToRed:r green:g blue:b alpha:1.0f];
}
...
+ (UIColor *)pp_colorWithHexString:(NSString *)stringToConvert {
	...
	return [UIColor pp_colorWithRGBHex:hexNum];
}

// Lookup a color using css 3/svg color name
+ (UIColor *)pp_colorWithName:(NSString *)cssColorName {
	UIColor * color = [[[self class] colorNameCache] objectForKey:cssColorName];
    
    if ([NSNull null] == (id)color) {
        color = nil;
    }
    else if (!color) {
        color = [self searchForColorByName:cssColorName];
        
        [[[self class] colorNameCache] setObject:((color) ? color : (id)[NSNull null]) forKey:cssColorName];
    }
...
@implementation UIColor (PPToolkitAdditionsInternal)
/*
...
+ (UIColor *)searchForColorByName:(NSString *)cssColorName {
	...
	if (found) {
		...
		if (sscanf(after, "%x", &hex) == 1) {
			result = [self pp_colorWithRGBHex:hex];
		}
	}
