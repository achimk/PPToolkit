//
//  PPLoadingTableViewController.m
//  PPToolkit
//
...
#import "PPLoadingTableViewController.h"

#import "PPTableViewCell.h"

NSString * const PPLoadingTableViewControllerFailureException       = @"PPLoadingTableViewControllerFailureException";

#pragma mark - PPLoadingTableViewController

@interface PPLoadingTableViewController ()
...
@implementation PPLoadingTableViewController
...
- (void)loadDataWithState:(PPLoadingState)state success:(void(^)(void))success failure:(void(^)(NSError *))failure {
    [NSException raise:PPLoadingTableViewControllerFailureException format:@"Failed to load data. Implement method: '%@' with state: %@", NSStringFromSelector(_cmd), PPStringFromLoadingState(state)];
}
